...
open("https://raw.githubusercontent.com/Aw-bree/seedHelper/master/update_products.json") do |productfiles|
  data = []
  ...
  data.each do |prod|
    ...
    @product.update_attributes!(title: prod[:title], fabric_stretch: prod[:fabric_stretch], fabric_material: prod[:fabric_material], main_fiber_content: prod[:main_fiber_content], care_advice: prod[:care_advice],care_instructions: prod[:care_instructions],description: prod[:description])
  end
end
